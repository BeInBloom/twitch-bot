...
impl TokenManager {
    ...
    async fn refresh_now(&self) -> Result<(String, u64)> {
        ...
        let full_token = format!("oauth:{}", response.access_token);
        ...
        let Some(new_refresh_token) = response.refresh_token else {
            return Ok((full_token, response.expires_in));
        };

        if new_refresh_token != current_refresh {
            warn!("twitch rotated the refresh token");

            {
                let mut refresh_token_lock = self.refresh_token.write().await;
                *refresh_token_lock = new_refresh_token.clone();
            }
        ...
        Ok((full_token, response.expires_in))
    }
...
mod tests {
    ...
    async fn test_refresh_token_stored_correctly() {
        let manager = make_manager();
        let refresh_token = manager.refresh_token.read().await;
